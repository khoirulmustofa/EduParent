...
const menuItems = ref([
    ...
    },
    {
        title: 'Kunjungan\nPerpustakaan',
        icon: libraryOutline,
        path: '/menu/library-visit', // Sesuaikan path jika perlu
        color: 'green'
    }
]);
</script>
        ...
        <ion-content :fullscreen="true" class="bg-gray-50 font-sans">
            ...
            <div class="relative z-10 px-5 py-2 space-y-2">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div v-for="(item, index) in menuItems" :key="index" @click="router.push(item.path)"
                        class="bg-white p-2 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 active:scale-95 transition-all cursor-pointer flex flex-col items-center justify-center gap-2 group h-full">
                        <div
                            class="w-12 h-12 rounded-2xl bg-green-50 text-green-500 flex items-center justify-center shadow-sm group-hover:bg-green-500 group-hover:text-white transition-colors">
                            <ion-icon :icon="item.icon" class="text-xl"></ion-icon>
                        </div>

                        <div class="text-center">
                            <div class="font-bold text-gray-700 !text-sm leading-tight whitespace-pre-line">
                                {{ item.title }}
                            </div>
                        </div>
                    </div>
